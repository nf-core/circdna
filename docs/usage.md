# nf-core/circleseq: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/circleseq/usage](https://nf-co.re/circleseq/usage)
...
```console
nextflow run nf-core/circleseq --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/circleseq
```
...
First, go to the [nf-core/circleseq releases page](https://github.com/nf-core/circleseq/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
