...
if sys.version_info < (3, 0) and mosek_major >= 10:
    logging.warning("Mosek version is " + ".".join([str(x) for x in mosek_ver]) + " which requires python3. Exiting.\n")
    sys.exit(1)
    ...
    with mosek.Env() as env:
        with env.Task() as task:
            ...
            if task.getsolsta(mosek.soltype.itr) != mosek.solsta.optimal:
                raise Exception(
                    "Failed to solve to optimality. Solution status {}".format(task.getsolsta(mosek.soltype.itr))
                )
